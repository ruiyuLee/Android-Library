...
public final class AsyncTaskAssistant {
     ...
     */
    private static class WorkderAsyncTask extends AsyncTask<Task, Object, Object> {
    ...
    public static void execute(final Runnable runnable, long delayTimeMS) {
        if (sHandler == null) {
            throw new RuntimeException("Please call AsyncTaskAssistant.init() on application "
                    + "ui thread before execute post delay tasks.");
        }
    ...
    public static void executeOnThreadPool(Runnable runnable) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            WorkderAsyncTask asyncTask = new WorkderAsyncTask();
            asyncTask.executeOnExecutor(WorkderAsyncTask.THREAD_POOL_EXECUTOR, task);
        } else {
            WorkderAsyncTask asyncTask = new WorkderAsyncTask();
